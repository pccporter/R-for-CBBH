...
## Datatypes
- The smallest object in R is called and atomic element.
...
```
## Check datatype
- If you do not know the datatype you can use the `class`-function
```{r}
typeof(a_number)
```
...
- If we try to mix two types `R` will automatically *cast* `logical`->`numeric`->`character`
...
A <- as.data.frame(cbind(rep(1:3, 4), rep(c(4, 3, 2, 1), 3)))
...
- Strings are usually converted to factors automatically (unless you use a data structure called a tibble).
...
as.integer(A[6,1]) + as.integer(A[3,2])
